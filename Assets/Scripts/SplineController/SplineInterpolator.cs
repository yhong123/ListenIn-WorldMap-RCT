...
public class SplineInterpolator : MonoBehaviour
{
	...
	internal class SplineNode
	{
		...
		internal SplineNode(Vector3 p, Quaternion q, Vector3 s, float t, Vector2 io) { Point = p; Rot = q; Scale = s; Time = t; EaseIO = io; }
		internal SplineNode(SplineNode o) { Point = o.Point; Rot = o.Rot; Scale = o.Scale; Time = o.Time; EaseIO = o.EaseIO; }
	}
	...
	public void StartInterpolation(OnEndCallback endCallback, bool bRotations, eWrapMode mode, bool useRigidBody, bool useScale)
	{
		if (mState != "Reset")
	...
	public void AddPoint(Vector3 pos, Quaternion quat, Vector3 scale, float timeInSeconds, Vector2 easeInOut)
	{
		if (mState != "Reset")
			throw new System.Exception("Cannot add points after start");

		mNodes.Add(new SplineNode(pos, quat, scale, timeInSeconds, easeInOut));
	}
	...
	void Update()
	{
		...
		if (mCurrentTime >= mNodes[mCurrentIdx + 1].Time)
		{
			...
			else
			{
				if (mState != "Loop")
				{
					...
					//Adding Scaling information
					if(mScaling)
						transform.localScale = mNodes[mNodes.Count - 2].Scale;
		...
		if (mState != "Stopped")
		{
			...
			currPosition = GetHermiteInternal(mCurrentIdx, param);
			...
			if(mScaling)
			{
				transform.localScale = GetLinearInterpolation(mCurrentIdx, param);
			}
		}
	}
	...
	void FixedUpdate()
	{
		if(mRigidBody)
		{
			gameObject.GetComponent<Rigidbody2D>().MovePosition(currPosition);
//			Vector2 lastPosition = gameObject.GetComponent<Rigidbody2D>().position;
//			Vector2 direction = (new Vector2(currPosition.x,currPosition.y) - lastPosition).normalized;
//			gameObject.GetComponent<Rigidbody2D>()
//				.MovePosition(lastPosition + direction * Time.fixedDeltaTime );
		}
	}

	public Vector3 GetLinearInterpolation(int idxFirstPoint, float t)
	{
		Vector3 returnScale;
		returnScale = ((mNodes[idxFirstPoint + 1].Scale - mNodes[idxFirstPoint].Scale) * t) + mNodes[idxFirstPoint].Scale;
		return returnScale;
	}
