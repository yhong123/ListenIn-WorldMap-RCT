...
public class MadLevelGridLayout : MadLevelAbstractLayout {
    ...
    public bool ignoreInputForIconsOnDifferentPages = false;
    ...
    private void UpdateIgnoreInput() {
        ...
        for (int i = 0; i < pages.Count; i++) {
            ...
            for (int j = 0; j < page.icons.Length; j++) {
                MadLevelIcon icon = page.icons[j];
                icon.GetComponent<Collider>().enabled = page == currentPage;
            }
        }
    }
                ...
                for (int x = 1; x <= gridWidth && levelIndex < levelCount; ++x, levelIndex++, levelPageIndex++) {
                    ...
                    if (createNewInstance) {
                        ...
                        levelIcon = CreateIcon(
                            page.transform, level.name, template);
                    } else {
