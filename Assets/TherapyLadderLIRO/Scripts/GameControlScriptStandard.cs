...
public class GameControlScriptStandard : MonoBehaviour 
{
    ...
    void Update()
    {
        ...
        if (enable_input)
        {
            ...
            else if (Input.GetMouseButtonDown(0))
            {
                ...
                currMask = LayerMask.NameToLayer("Stimulus");
                RaycastHit2D hitInfo = Physics2D.Raycast(Camera.main.ScreenToWorldPoint(pos), Vector2.zero, Mathf.Infinity, currMask);
                if (hitInfo)
                {
                    ShowFeedbackLIRO(hitInfo);
                }
    ...
    void ShowFeedbackLIRO(RaycastHit2D hitInfo)
    {
        ...
        if (m_arrStimulusGO[m_intSelectedStimulusIdx].stimulusScript.m_registeredID == currChallenge.ChallengeID)
        {
    ...
    void PrepareNextTrialLIRO()
    {
        ...
        currChallenge = m_currListOfChallenges[m_curChallengeIdx];
        RandomizeFoils(currChallenge);
    ...
    {
        for (var i = 0; i < currChallenge.Foils.Count; ++i)
        {
    ...
    public void ShowNextTrialLIRO()
    {
        ...
        for (var i = 0; i < currChallenge.Foils.Count; ++i)
        {
            switch (currChallenge.Foils.Count)
            {
                case 3:
            ...
            m_arrStimulusGO[i].stimulusScript.SetStimulusImage("Images/phase1/" + currChallenge.Foils[i].ToString());
            m_arrStimulusGO[i].stimulusScript.m_registeredID = currChallenge.Foils[i];
    ...
    private void RandomizeFoils(Challenge currChallenge)
    {
        for (int i = 0; i < currChallenge.Foils.Count; i++)
        {
            long temp = currChallenge.Foils[i];
            int intRandomIndex = UnityEngine.Random.Range(i, currChallenge.Foils.Count);
            currChallenge.Foils[i] = currChallenge.Foils[intRandomIndex];
            currChallenge.Foils[intRandomIndex] = temp;
        }
    }
    ...
    public float PlayAudioLIRO(float fDelay = 0)
    {
        ...
        string strAudio = "Audio/phase1/" + currChallenge.FileAudioID;
